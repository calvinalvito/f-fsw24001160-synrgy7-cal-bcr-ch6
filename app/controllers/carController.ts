...
//Delete Data Car
export const deleteCar = async (req: CustomRequest, res: Response): Promise<void> => {
  try {
    const { id } = req.params;
    const carId = parseInt(id, 10);
    console.log(carId);
  ...
  try {
    mUpload.single('picture')(req, res, async (err: any) => {
      ...
      try {
        ...
        // Mencatat aktivitas log penciptaan mobil
        const userId = req.user.id; // Mengambil ID pengguna dari token JWT
        await CarLogService.createLog({ activity_type: 'create', user_id: userId, car_id: car.id });
  ...
  try {
      mUpload.single('picture')(req, res, async (err: any) => {
          ...
          try {
              ...
              // Mencatat aktivitas log pembaruan mobil
              const userId = req.user.id; // Mengambil ID pengguna dari token JWT
              await CarLogService.createLog({ activity_type: 'update', user_id: userId, car_id: carId });
